...
static int xd3_emit_double (xd3_stream *stream, xd3_rinst *first, xd3_rinst *second, uint8_t code);
...
#if XD3_ENCODER
inline int xd3_emit_byte (xd3_stream  *stream, xd3_output **outputp, uint8_t code)
{
...
inline int xd3_emit_bytes(xd3_stream*stream, xd3_output **outputp, const uint8_t *base, usize_t size)
{
  xd3_output *output = (*outputp);
...
static void* __xd3_alloc_func (void* opaque, size_t items, usize_t size) { return malloc (items * (size_t) size); }
...
int xd3_set_source_and_size(xd3_stream *stream, xd3_source *user_source, xoff_t source_size)
{
...
{
  IF_DEBUG3(DP(RINT "[xdelta3] xd3_encode_buffer_leftover\n"));

  /* Allocate the buffer. */
  if (!stream->buf_in)
  {
    IF_DEBUG3(DP(RINT "[xdelta3] allocating buf_in of %lu\n", stream->winsize));
    stream->buf_in = (uint8_t*)xd3_alloc(stream, stream->winsize, 1);
  ...
  usize_t room = stream->winsize - stream->buf_avail;
  usize_t take = xd3_min (room, stream->avail_in);
...
{
  IF_DEBUG3(DP(RINT "[xdelta3] xd3_encode_input\n"));
  int ret, i;
  ...
  switch (stream->enc_state)
  {
    case ENC_INIT:
      ...
      IF_DEBUG3(DP(RINT "[xdelta3] ENC_INIT -> ENC_INPUT\n"));
    ...
    case ENC_INPUT:
      ...
      {
        IF_DEBUG3(DP(RINT "[xdelta3] ENC_INPUT -> return XD3_INPUT\n"));
        return XD3_INPUT;
      }
    ...
    enc_flush:
      ...
      if ((stream->buf_leftover != NULL) || (stream->buf_avail != 0) || (stream->avail_in < stream->winsize && ! (stream->flags & XD3_FLUSH)))
      {
